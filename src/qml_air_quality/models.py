...
def make_logistic(seed: int = 42, max_iter: int = 2000) -> LogisticRegression:
...
def make_linear_svm(seed: int = 42) -> CalibratedClassifierCV:
    base = SVC(kernel="linear", class_weight="balanced", random_state=seed)
    return CalibratedClassifierCV(base, method="sigmoid", cv=3)


def make_rbf_svm(
    seed: int = 42, C: float = 1.0, gamma: str | float = "scale"
) -> CalibratedClassifierCV:
    base = SVC(kernel="rbf", C=C, gamma=gamma, class_weight="balanced", random_state=seed)
    return CalibratedClassifierCV(base, method="sigmoid", cv=3)
